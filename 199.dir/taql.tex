...
be used at will.
\\\htmlref{Section 7 (Modifying a table)}{TAQL:MODIFYING} explains the
UPDATE, INSERT, and DELETE commands in more detail. The CREATE TABLE command is
explained in \htmlref{section 8 (Creating a table)}{TAQL:CREATETABLE}.
\\\htmlref{Section 9 (Counting in a table)}{TAQL:COUNTING}
explains the COUNT command in more detail.
\\\htmlref{Section 10 (Calculations on a table)}{TAQL:CALCULATING}
...
\\Joins are explained further in \htmlref{section 6.1}{TAQL:JOIN}.
...
detail in \htmlref{section 5}{TAQL:AGGREGATION}.
...
\\Aggregate functions are discussed in more detail in
   \htmlref{section 5}{TAQL:AGGREGATION}. 
...
functions are used.
\\It is discussed in more detail in \htmlref{section 5}{TAQL:AGGREGATION}.
...
However, there are several aggregate functions returning an array.
The last three functions (GHIST, GAGGR, and GROWID) return an array.
...
\begin{verbatim}
  ...
  GMEANS  GVARIANCES  GSTDDEVS  GRMSS
\end{verbatim}
...
\begin{description}
  ...
  \item[] \texttt{integer ROWID()}\\
    ...
    and \htmlref{examples in section 11.1}{TAQL:SELEXAMPLES}).
...
in \htmlref{section 3.10}{TAQL:GIVING}). E.g.
\begin{verbatim}
...
\htmlref{section 4.10.13}{TAQL:AGGRFUNC} can be used.
For example:
\begin{verbatim}
