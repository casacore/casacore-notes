...
TaQL contains several commands.
...
\begin{verbatim}
  ALTER TABLE table
    ...
    [ADD ROW nrow]
\end{verbatim}
It can be used to add, rename, and remove columns and keywords and to
add rows.
...
needed. Multiple tables require a shorthand and are useful when:
\begin{itemize}
...
query command by using the shorthand given to that table. It can also
...
\item An SQL-style pattern can be given directly as a string constant
  preceded by operator LIKE or NOT LIKE.
...
\begin{tabular}{lp{2cm}p{10cm}}
 ...
 resulting in an integer, thus \texttt{1./2.} results in \texttt{0} \\
  ...
  NOT LIKE & string & test if string does not match an SQL pattern. \\
...
A unit can be preceded by a scaling prefix (like \texttt{k} for kilo).
Compound units are created when multiplying or
dividing values with units. The available units can be shown using the
...
\end{verbatim}
 ...
 \item Division of units of an equal kind (e.g., km by m) results in a
   unitless value.
 \item Division of a unitless value by a value with unit, results in
   the reciproke unit. 
\begin{verbatim}
...
\begin{description}
  ...
  \item[] \texttt{DateTimearray MEAS.RISESET(direction, epoch, position)}\\
   returns the rise and set date/times (UTC) of the sources given in the
    direction argument for the given dates and positions. Function
    name MEAS.RISET can be used as well.
...
\end{description}
The available reference types can be shown using the
{\tt show meastypes} command in the program {\em taql}.
For ease of use several specialized MEAS.DIR and MEAS.POS functions
...
\end{description}
The names of the last two functions have a suffix xxx indicating how
...
\item 'position' gives one or more directions to use. They can be
  ...
  possible to distinguish between LL and XYZ by using suffices like
  XYZ or LL in the reference type given in the next argument.
  \begin{itemize}
    \item As a scalar or array of observatory names using their positions
      in the Measures Observatory table.
...
Note that TaQL has no way of showing keywords that have a record value.
The program {\em showtable} can be used for that purpose.
...
\subsection{Program \texttt{taql}}
  ...
  \item The program is run interactively if command nor -f is given.
    ...
    \begin{itemize}
    \item \texttt{help}, \texttt{h} or \texttt{?} shows brief help
      information.
    \item\texttt{show} can be used to show to available units or
      measure types.
    \item A full TaQL command. If no GIVING part is given, the
      resulting table is not made persistent. It shows the number of matching rows.
...
\begin{itemize}
  ...
  \item Handle invalid subexpressions (e.g., exceeding array bounds)
    as null arrays
    which can be tested with the function ISNULL.
\end{itemize}
