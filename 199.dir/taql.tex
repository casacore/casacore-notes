...
data in Casacore tables. It makes it possible to get results or select
...
\item In the WITH clause the order of the table and alias is reversed
  compared to SQL because it was felt as not logical having it
  different from the FROM clause.
\end{itemize}
...
indicating with table in the FROM clause has to be used. If not
...
unit after a (sub)expression. E.g., \texttt{3deg rad} converts 3 degrees to radians. 
...
whether whitespace is used between a value and simple unit.
Without whitespace the unit is directly part of the value, with
whitespace it is seen as a subexpression. This is relevant in two cases.
\begin{enumerate}
\item Without whitespace a conversion unit can be given without
  parentheses. With whitespace parentheses are needed around the
  subexpression.  Compound units are always seen as a subexpression.
\item A few units are identical to reserved TaQL keywords (e.g., 'in' for
inch or 'as' for arcsecond). Such units have to be quoted or escaped
with a backslash, unless given after a value without whitespace.
\end{enumerate}
For example:
\begin{verbatim}
  10deg rad         # is fine
  (10 deg)rad       # is fine
  10 deg rad        # gives a parse error
  (10'm/s')'km/h'   # is fine       
  10'm/s' 'km/h'    # gives a parse error
  1in               # 1 inch
  1 \in             # 1 inch
...
epochs, positions, earthmagnetics and frequencies from one reference
frame to another. All conversions supported by Casacore's 
...
TaQL can handle the following types of measures. A measure can be
converted from one reference frame to another.
\begin{itemize}
\item Positions on the earth in ITRF or WGS84 coordinates defined as
  xyz or as longitude-latitude-height. The positions of many observatories
  are known.
\item Epochs in various coordinate frames, typically UTC is used. A
  conversion might require a Position.
\item Directions at the sky in various coordinate frames as angles
  (e.g., RA,DEC) or as direction cosines. The directions of many
  sources (also planets) are known. A conversion might require
  an Epoch and Position. Rise/set times of sources can be calculated.
\end{itemize}
...
\\The function parameters can be single values, but also arrays of values.
A function will operate on each element of the Cartesian product of these arrays.
\begin{description}
  \item[] \texttt{doublearray MEAS.POS(toref, position)}\\
    \texttt{doublearray MEAS.POSITION(toref, position)}\\
    ...
    the result.
    \\ - None or XYZ returns as xyz (unit m).
    \\ - LLH returns as lon-lat-height (in rad,rad,m but without units).
    \\ - LL or LONLAT returns as lon-lat (unit rad).
    \\ - H or HEIGHT returns as height (unit m).
  ...
  \item[] \texttt{doublearray MEAS.DIR(toref, direction, epoch, position)}\\
    \texttt{doublearray MEAS.DIRECTION(toref, direction, epoch, position)}\\
    ...
    needs frame information (e.g., when converting J2000 to apparent).\\
  ...
  \item[] \texttt{DateTimearray MEAS.RISET(direction, epoch, position)}\\
    \texttt{DateTimearray MEAS.RISESET(direction, epoch, position)}\\
    ...
    and one of the following case-insensitive suffices indicating what
    to use. The default is CR which is used in most almanacs.
    \begin{itemize}
...
\end{description}
The possible reference frame types can be shown using the
{\tt show meastypes} command in the program {\em taql}.
\\For ease of use several functions
are defined with an implicit 'toref' argument.
\begin{description}
...
\paragraph*{}
The function arguments can be given in a variety of ways. The argument
data types and units are used to distinguish the arguments.
\begin{itemize}
...
\end{verbatim}
calculates the local apparent sidereal time for the given date and
position. 
\begin{verbatim}
...
\end{verbatim}
calculates the azimuth/elevation (in degrees) of the given source direction for
the LOFAR site for 24 hours. The result is an array with shape [24,2].
...
\subsection{Program \texttt{taql}}
  ...
  \item The program is run interactively if command nor -f is given.
