...
functions. User defined functions written in C++ are
supported, which is used to support coordinate conversions in TaQL.
TaQL also makes sorting and column selection possible.
Furthermore, TaQL has commands to update, add or delete rows, columns
and keywords in a table and to create, restructure or delete a table.
...
using C++ or the program \texttt{taql}. 
...
\item Only support for left joins with a single match. Multiple tables
  in a clause are not fully joined (as SQL does), but 
  joined on rowid. Hence they must have the same size.
...
\item The language can be extended by means of User Defined
  ...
  measure conversions (for direction, epoch, position, stokes,
  frequency, doppler, radialvelocity and earthmagnetic).
...
\begin{verbatim}
  CREATE TABLE table [AS options]
    [LIKE table [DROP COLUMN column_list]]
...
\begin{verbatim}
  ALTER TABLE table
    ...
    [COPY COLUMN colnew=col, colnew=col, ... [DMINFO datamanagers]]
...
\begin{itemize}
  \item add, copy, rename, and remove columns and keywords
  \item add rows (that have to be filled later)
\end{itemize}
...
\paragraph*{}
The clauses and verbs in the commands are
...
\\The DROP TABLE command is explained in 
\htmlref{section \ref{TAQL:DROPTABLE} (Removing a  table)}{TAQL:DROPTABLE}
...
It is possible to get some tracing output during the execution of a TaQL
command by using the case-insensitive value \texttt{TRACE} in the
\texttt{using style} clause. It can be useful for debugging purposes.

\subsubsection{Timing}
It is possible to time a TaQL command by using the case-insensitive
value \texttt{TIME} in the \texttt{using style} clause. 
For historical reasons it is also possible to use the the case-insensitive
keyword \texttt{TIME} before or after the optional style clause.
...
\begin{verbatim}
  ADDCOLUMN     ADDROW
  COPYCOLUMN    COPYKEYWORD
  DELETECOLUMN  DELETEKEYWORD
  DROPCOLUMN    DROPKEYWORD    DROPTABLE
  RENAMECOLUMN  RENAMEKEYWORD
                SETKEYWORD
\end{verbatim}
Note that the words in the TaQL vocabulary are case insensitive, 
...
The JOIN clause can be used to left join a table with another table.
It also is possible to join tables on row number, where
the tables involved must have the same number of rows.
One can also join on row number the main table of a
MeasurementSet with a subtable such as the ANTENNA table using a
...
\begin{verbatim}
  ...
  SELECT [[DISTINCT] column_list]
    [INTO table [AS options]]
    [FROM table_list]
    [JOIN table_list ON condition [JOIN table_list ON condition ...]]
    [WHERE expression]
    [GROUPBY expression_list]
    [HAVING expression]
    [ORDERBY [DISTINCT] sort_list]
...
\item FROM to define the main tables to be used.
\item JOIN to combine main tables with other tables.
...
\item If preceded by a shorthand (like in \texttt{t0.DATA}), the name
  is looked up in the corresponding table given in the FROM or JOIN clause.
...
A selection results in a so-called reference table. Optionally a
...
indicating which table in the FROM or JOIN clause has to be used. If not
given, a column will be looked up in the first FROM table. Note that if
...
Note that for subqueries the \htmlref{GIVING}{TAQL:GIVING} clause offers a
...
reference table, a plain table, or a memory table. It can also be a
subtable of an existing table using the :: notation. For example:
\begin{verbatim}
  INTO my.tab::SUBTAB
\end{verbatim}
creates subtable SUBTAB in parent table 'my.tab'. It also defines the keyword
SUBTAB in the parent table to refer to the subtable.
...
It is also possible to store the result in a subtable of another table
using the :: notation, which is similar to specifying an
\htmlref{input subtable}{TAQL:TABLE_LIST} as described in the next
subsection. This will also create the keyword in the main table
referring the subtable.
For example:
\begin{verbatim}
  GIVING my.ms::NEWSUB
\end{verbatim}
will create subtable \texttt{NEWSUB} of table \texttt{my.ms} and define
keyword \texttt{NEWSUB} to refer to the subtable.
...
column-independent expressions (such as function random) 
in the SELECT command. Note that these
...
original table. It also fails if a table is stored in the MultiFile format.
...
command in the FROM or JOIN clause. This is a normal query command
enclosed in square brackets or parentheses. Besides the SELECT command
the COUNT, CREATE TABLE and ALTER TABLE commands can also be used.
...
\\Finally note that the new \htmlref{WITH clause}{TAQL:WITH} 
...
Usually the result of a TaQL query references the first table given in the FROM.
...
TaQL has various ways of joining tables which are described in the
next sections.
\begin{itemize}
\item
The JOIN clause it the most versatile way and can be used to perform
left joins, where the rows in the main table are matched against a row
in the join table on one or more columns. If multiple rows in the join
table match, the first one is taken.
Joins are explained in more detail in \htmladdnormallink{this article}
{https://learnsql.com/blog/sql-joins-types-explained}.
\item
Multiple tables can be given in the FROM or JOIN clause. They are
implicitly joined on row number. if a column is used from them. Hence
they must have the same size.
If only a keyword is used from a table, that table is not joined.
\item
A value vector, returned by  a subquery, can be indexed. In this way
the ANTENNA1 column in a MeasurementSet can be used as index in the
vector of antenna names.
\item
The special \htmlref{\texttt{derivedmscal}}{TAQL:MSCAL} functions make it easy to
obtain information from the subtables of a MeasurementSet or
Calibration Table.
\end{itemize}

\subsubsection{JOIN clause}
The JOIN clause can be used to left join tables in order to find (meta)
data. For example, a MeasurementSet can be joined with its ANTENNA
subtable to find the name or position of the antenna in each row in
the MeasurementSet. Columns from the join table can be used in any
other clause (e.g., WHERE) in a TaQL command.
\\TaQL only supports left joins. These are joins where the rows in the
left (the main) table are matched against a single row in
the joined table. If no matching row is found, 'none' values are used
if data from the join table is used. If multiple rows match, the first
matching row is used. Other types of joins (such as full join)
are not supported.

The full  syntax is:
\begin{verbatim}
  JOIN table-list ON join-expression
\end{verbatim}
Multiple JOIN clauses can be given to join with multiple tables. It is
also possible to join a joined table with another table. This is shown
in an example below.

The table-list is similar to that in the FROM clause, thus one or more
table names followed by a shorthand. If multiple tables are given,
they must have the same number of rows. The tables are used in
the join expression, so usually a shorthand is needed.
\\The join-expression tells how to join tables. It is an expression
consisting of one or more subexpressions separated by AND.
Each subexpression compares a column in one table with a column (or
the row number)  in the join table. 
For example for MSv3:
\begin{verbatim}
  FROM my.ms t1 JOIN ::ANTENNA t2 ON t1.ANTENNA1=t2.ANTENNA_ID
\end{verbatim}
For each row in \texttt{my.ms} it finds the row in the ANTENNA subtable
matching ANTENNA1. In the SELECT clause \texttt{t2.NAME} can be used
to get, for instance, the name of the antenna like:
\begin{verbatim}
  SELECT t1.ANTENNA1, t2.NAME FROM my.ms t1
    JOIN ::ANTENNA t2 ON t1.ANTENNA1=t2.ANTENNA_ID
\end{verbatim}
Another join can be used to get the name for ANTENNA2.
\begin{verbatim}
  SELECT t1.ANTENNA1, t2.NAME as ANTNAME1,
         t1.ANTENNA2, t3.NAME as ANTNAME2 FROM my.ms t1
    JOIN ::ANTENNA t2 ON t1.ANTENNA1=t2.ANTENNA_ID
    JOIN ::ANTENNA t3 ON t1.ANTENNA2=t3.ANTENNA_ID
\end{verbatim}

In MSv2 quite often the row number is used as an implicit id. This can also
be used in a join like:
\begin{verbatim}
  SELECT t1.ANTENNA1, t2.NAME FROM my.ms t1
    JOIN ::ANTENNA t2 ON t1.ANTENNA1=t2.rownumber()
\end{verbatim}

In MSv3 an explicit ANTENNA\_ID is used instead of a row number.
To make it possible to use the same query for an MSv2 and MSv3,
the \texttt{MSID} function can be used for a column in a table.
It takes a column name as argument.
If the column exists, it will be used. Otherwise the
\texttt{rownumber} function is used. A shorthand can be given before
the column name. For example:
\begin{verbatim}
  SELECT t1.ANTENNA1, t2.NAME FROM my.ms t1
    JOIN ::ANTENNA t2 ON t1.ANTENNA1=msid(t2.ANTENNA_ID)
\end{verbatim}

Besides joining using the equality operator, it is also possible to
join using an interval in the join table. For example:
\begin{verbatim}
  FROM my.ms t1
    JOIN ::SYSCAL t2 ON t1.TIME AROUND t2.TIME IN t2.INTERVAL
\end{verbatim}
In this example the SYSCAL subtable contains information for time
intervals defined by its center and width. The AROUND/IN interval
specification can be used to find the matching row for each TIME in
the main table of the MS.

Alas, finding SYSCAL information is not as easy as shown here, because
it also depends on antenna and spectral window. So the full join is
more complex. For MSv2 it should look like:
\begin{verbatim}
  FROM my.ms ms
    JOIN ::DATA_DESCRIPTION dd ON ms.DATA_DESC_ID=dd.rownumber()
    JOIN ::SYSCAL sc ON dd.SPECTRAL_WINDOW=sc.SPECTRAL_WINDOW_ID
                    AND ms.ANTENNA1=sc.ANTENNA_ID
                    AND ms.TIME AROUND sc.TIME IN tsc.INTERVAL
\end{verbatim}
First a join with the DATA\_DESCRIPTION subtable is done to find the
spectral window. Thereafter a join with the SYSCAL subtable is done
using the antenna, spectral window and time.
 
\subsubsection{Join on row number}
Multiple tables can be given in the FROM or JOIN clause. They are
implicitly joined on row number as long as a column is used from such
a table. Hence they must have the same size.
If only a keyword is used from a table, that table is not joined.
\begin{verbatim}
...
\subsubsection{\label{TAQL:MSCAL}Join using derivedmscal}
Several \htmlref{UDF's in the \texttt{derivedmscal} library}{TAQL:MSFUNC} make it possible to
...
In the main query and in a query in the FROM or JOIN clause the
...
the tables specified in the FROM or JOIN clause. However, it is possible to
...
\begin{verbatim}
  ...
  *  /  // %
  ...
  ^
  |
  == != >  >= <  <=  ~= !~= IN INCONE BETWEEN AROUND EXISTS (I)LIKE  ~  !~
  &&
  ||
\end{verbatim}
...
\begin{tabular}{lp{2cm}p{10cm}}
 ...
 resulting in an integer, thus \texttt{1./2.} results in \texttt{0} \\
  ...
  NOT BETWEEN & no bool & \texttt{x NOT BETWEEN b AND c} is the negation
  of above. \\
  AROUND & real,datetime & \texttt{x AROUND mid IN width} is similar to
  \texttt{x>=mid-width/2 AND x<=mid+width/2} and \texttt{x IN [mid<:>width]} \\
  NOT AROUND & no bool & \texttt{x NOT AROUND mid IN width} is the negation
  of above. \\
...
It can define multiple discrete values and a continuous interval. 
The possible forms of a set element are:
\begin{enumerate}
\item A single value as shown in the example above.
\item \texttt{start:end:incr}. This specifies multiple discrete values
...
\end{verbatim}
\item Continuous real and datetime intervals can also be specified as
\texttt{mid<:>width}, thus
  by giving the middle and the width of an interval. Such an interval
  is bounded and closed on both sides. For a datetime
  interval the middle must be given as a datetime and the interval as
  a real with a possible unit which defaults to d (days).
  If the width is zero, the interval is unlimited, thus basically
  -infinity till +infinity.
  This way of specifying an interval is useful
  because the intervals in a MeasurementSet are defined that way.
For example:
\begin{verbatim}
  23.5<:>5          means 21<=x<=26
  19Apr2022<:>10    means 14Apr<=x<=24Apr2022
\end{verbatim}
\item Intervals in a set can also be given using BETWEEN/AND and
  AROUND/IN. For example:
\begin{verbatim}
  column IN [BETWEEN 10 AND 20, AROUND 35 WIDTH 10]
\end{verbatim}
which is equal to
\begin{verbatim}
  column IN [10=:=20, 35<:>10]
and
  column BETWEEN 10 AND 20 OR column AROUND 35 IN 10
\end{verbatim}
Thus the BETWEEN/AND and AROUND/IN syntax can be used directly to define a
single interval or be used to define intervals in a set.

\end{enumerate}
It is very important to note that the 2nd form of set specification results in
discrete values, while the 3rd and later forms result in continuous intervals.
...
\\By default a value is
right adjusted, but can be left adjusted by giving a negative width.
\\A bool value is prettyfied as 'True' or 'False'. Using format '\%d'
it is prettyfied as 1 or 0.
\begin{itemize}
\item A numeric format value defines the width and/or precision. For example:
\begin{verbatim}
  8        defines width 8 and default precision
  -8       the same, but left adjusted
  20.12    defines width 20 and precision 12
  .8       defines precision 8 and default width
\end{verbatim}
  Note that precision represents all digits, not only the ones behind
  the decimal point. Thus 10.3 is not the same as '\%10.3d' as the
  latter defines 3 decimals.
\item A string format value can contain a \texttt{printf}-style format
  ...
  {https://www.cplusplus.com/reference/cstdio/printf} for possible
  format specifiers.
  For example:
\begin{verbatim}
  %10d     decimal with width 10 (right adjusted)
  %-10d    decimal with width 10 (left adjusted)
  %010d    decimal with width 10 and filled with zeroes
  %10.3f   defines width 10 and 3 decimals
...
\end{verbatim}
  ...
  A vertical bar or a comma (with optional whitespace) must be used as
  separator; they cannot be mixed.
  A  numeric value can be part of the string to define the precision of the
  time/angle. The default precision is 6 (thus hh:mm:ss).
...
\begin{tabular}{ll}
 ...
 ISO & yyyy-mm-dd hh:mm:ss.sssZ (same as FITS$|$USE\_Z$|$USE\_SPACE$|$CLEAN)\\
 ...
 LOCAL & local time; in FITS mode append time zone
  as +hh:mm\\
\end{tabular}
Note that giving a width for a date/time or angle defines the number
of digits in the time or angle part.
...
\begin{description}
  ...
  \item[] \texttt{real ABS(numeric),  real AMPLITUDE(numeric), real AMPL(numeric)}
...
\end{verbatim}
In this case \texttt{othertable} contains the time range for each windspeed.
...
\begin{verbatim}
  select from MY.MS t1
    JOIN ::DATA_DESCRIPTION t2 ON t1.DATA_DESC_ID=t2.rownumber()
    JOIN ::SPECTRAL_WINDOW t3 ON t2.SPECTRAL_WINDOW=t3.rownumber()
  where any(sqrt(sumsqr(UVW[:2])) / c() * t3.CHAN_FREQ < 100)
\end{verbatim}
...
\\The first join finds the SPECTRAL\_WINDOW\_ID for each row; the second
join finds the channel frequencies.
\\A \htmlref{derivedmscal}{TAQL:MSFUNC} function can be used for an
easier solution as shown below.
...
\subsection{\label{TAQL:UPDATE}UPDATE}
\begin{verbatim}
...
The syntax is:
\begin{verbatim}
  CREATE TABLE tablename AS options
     LIKE othertable AS shorthand DROP COLUMN columnlist
     ADD COLUMN colspecs
     LIMIT nrows
     DMINFO datamanagers
\end{verbatim}
...
\begin{itemize}
  \item The table name and options can be given in the same way as in the
    \htmlref{GIVING}{TAQL:INTO} clause. Thus it is also possible to
    create a subtable of an existing table. If no table name is given, a
    temporary table is created.
  \item The LIKE part can be used to create the new table with the
    same description as \texttt{othertable}. It is possible to drop some
    columns using the optional DROP COLUMN part. The \texttt{columnlist}
    consists of column names separated by commas. Of course,
    columns can be added using the ADD COLUMN part. Note that the
    shorthand of \texttt{othertable} can be used in the ADD COLUMN part.
  \item The columns to add are defined in the \texttt{colspecs} part. If not given, a
    table without any column is created. Below column specification is
    described in more detail.
    The words ADD COLUMN can be omitted if the LIKE part is not used,
    which makes it SQL-compatible.
...
\begin{verbatim}
  columnname LIKE othercolumn
             datatype [NDIM=n, SHAPE=[d1,d2,...], DIRECT=0/1, UNIT='s',
                       DMTYPE='s', DMGROUP='s', COMMENT='s']
\end{verbatim}
A column specification describes the column and consists of various parts.
\begin{itemize}
\item \texttt{columnname} gives the (alphanumerical) name of the column.
\item The optional LIKE part can be used to tell that the default column
  description must be copied from \texttt{othercolumn}. If
  \texttt{t.othercolumn} is given, the \texttt{othercolumn} is taken from the
  table with shorthand 't' as given in the WITH part. It is possible to
  override parts of the description by giving properties using the
  following parts.
\item The data type must be given if 'LIKE othercolumn' is not used,
  otherwise it can be given to override the data type taken from the
  \texttt{othercolumn}.
  The possible data type strings are given in
  \htmlref{section \ref{TAQL:DATATYPESTRING}}{TAQL:DATATYPESTRING}.
\item The last part (enclosed in square brackets) can be used to
  define other properties of the column. Zero or more of
  these keywords can be used. It makes it possible
  to define units, array columns and/or default data manager to be used.
...
thus must be enclosed in quotes. 
\item \texttt{COMMENT} defines comments for the column.
\\It has a string value, thus quotes
have to be used.
\item \texttt{DMTYPE, DMGROUP} are rather specific and are for the
  expert user to specify data managers.
...
\section{\label{TAQL:DROPTABLE}Removing a new table}
TaQL can be used to remove one or more tables. If a plain table is removed, its
subtables are removed as well.
The syntax is:
\begin{verbatim}
  DROP TABLE table-list
\end{verbatim}
The table-list is a comma separated list of table names, which can
also use :: to denote subtables.
\begin{verbatim}
  DROP TABLE my.ms, that.ms::EXTRA
\end{verbatim}
...
\begin{verbatim}
  ...
  COPY COLUMN new=col, new=col, ... DMINFO datamanagers
  RENAME COLUMN old TO new, old TO new, ...
  DELETE COLUMN column_list
  SET KEYWORD key=value AS dtype, ...
  COPY KEYWORD new=key AS dtype, ...
...
\begin{verbatim}
  COPY COLUMN newcola=cola, newcolb=colb, ... DMINFO datamanagers
\end{verbatim}
makes it possible to copy the data in a column to a new
column, that may not exist yet. If residing in different tables,
they must have the same number of rows.
The new column gets its description from the input column.
The optional DMINFO part can be used to define the datamanager(s) for
the new columns.
\\Note that copying to existing columns can be done with the
\htmlref{UPDATE}{TAQL:UPDATE} command. 
For example:
\begin{verbatim}
  ALTER TABLE mytab COPY COLUMN NCol1=Col1
  ALTER TABLE mytab FROM othertab t COPY COLUMN NCol1=t.Col1
  UPDATE mytab NCol1 SET NCol1=Col1
\end{verbatim}
The first example creates column \texttt{NCol1} taking its
description from \texttt{Col1}; thereafter the contents of
\texttt{Col1} are copied into it.
\\The second example is similar, but takes \texttt{Col1} from the table with
shorthand 't', which is given in the FROM part.
Because the data are copied, \texttt{mytab} and \texttt{othertab} must
have the same number of rows.
\\The third example is similar to the first one, but has to be used if
\texttt{NCol1} already exists.
...
\begin{description}
  ...
  \item[] \texttt{ SELECT FROM mytable WHERE object == pattern("3C*") \&\&}
   ...
   \item[] \texttt{ SELECT ANTENNA1,ANTENNA2,sqrt(sumsqr(UVW[:2]))}
     ...
     coordinate giving the projected baseline length.

   \item[] \texttt{ select NAME, sqrt(sumsqrs([select POSITION from my.ms/ANTENNA] -}
    \\\verb=    =
    \texttt{      resize(POSITION,[[select gcount() from my.ms/ANTENNA][0],3],1), 1))}
    \\\verb=    =
    \texttt{ from my.ms/ANTENNA}
    \\yields a matrix of the lengths of all baselines in \texttt{my.ms}.
     \\- The first inner select gets the POSITION of all antennae from
     which the POSITION in each row is subtracted (giving all baselines for each antenna).
     \\- The second inner select gets the numbe r of antennae. It is used in the resize function (using mode 1 to copy the values) to make the arrays in the subtraction the same shape. TaQL does not do axes broadcasting as numpy does.
     \\- The sumsqrs function calculates x**2+y**2+z**2 for each baseline (it collapses axes 1), whereafter sqrt gives their lengths.
...
\subsection{Program \texttt{taql}}
  ...
  \item A full TaQL command preceded by \texttt{varname=}, where
    ...
    because it is the very same as selecting multiple column
    expressions in TaQL's SELECT command.
  ...
  \item A TaQL command can be preceded by options to override the
    ...
    giving options on a command line, they are persistent, thus
    new global settings are defined.
