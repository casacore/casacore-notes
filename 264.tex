...
This table proposed in v2.0 but we explicitly define it here. It is referenced from the FEED table or from the newly proposed PHASED\_ARRAY table here. 
...
PHASED\_ARRAY & phased array stations information & PHASED\_ARRAY\_ID, ANTENNA\_ID \\
...
\item[FEED$n$] Feed number ($\geq 0$). For $n > 2$, triple-product
  ...
  phased\_array is given in the PHASED\_ARRAY subtable.
...
\definetable{PHASED\_ARRAY: phased array stations information}{
\defline{\bf Columns}
\defline{\em Key}
PHASED\_ARRAY\_ID & Int & & & phased array id.\\
\defline{\em Data}
ANTENNA  &   Int(\na)  & & & List of antenna ids\\
FEED\_ID     &   Int(\na)  & & & List of Feed id used on each antenna\\
DIRECTION & Double & rad & DIRECTION & Direction on sky where phased array has ccentered beam\\
...
\item[ANTENNA] list Antenna identifier, as indexed from ANTENNA table, that is used to form this array

\item[FEED\_ID] list of feed for each antenna used in array

\item[DIRECTION] Direction of beam phased on the sky.
