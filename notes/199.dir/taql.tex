...
\item Limited support for joins. Only implicit joins on row number are supported.
...
its name as a string (e.g., 'DELAY\_DIR'). 
...
\begin{verbatim}
   derivedmscal.azel1('\MOON')
\end{verbatim}
means that column MOON in the FIELD table has to be used.
...
\begin{description}
 \item[] \texttt{complexarray DERIVEDMSCAL.STOKES(complexarray, string)}\\
   converts the data.
 \item[] \texttt{doublearray DERIVEDMSCAL.STOKES(doublearray, string)}\\
   combines the weights.
 \item[] \texttt{boolarray DERIVEDMSCAL.STOKES(boolarray, string)}\\
   combines the flags.
\end{description}
