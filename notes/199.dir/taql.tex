...
The function \texttt{arraydata}, \texttt{arraymask}, and \texttt{flatten}
...
\begin{description}
  ...
  \item[] \texttt{doublearray MSCAL.UVW()}\\
...
\htmladdnormallink{ CASA baseline selection syntax}
{http://www.aoc.nrao.edu/~sbhatnag/misc/msselection/msselection.html}
\begin{verbatim}
  select from my.ms where mscal.baseline('RT[2-4]')
\end{verbatim}
selects the baselines containing an antenna whose name matches the pattern in the
function argument. Note that autocorrelations are not selected this
way.

Similar functions \texttt{TIME, FIELD, SCAN, SPW, UVDIST, STATE, OBS}, and
\texttt{ARRAY} use the CASA selection syntax to select on
the corresponding fields.
The aforementioned document also describes these syntaxes. For
example:
\begin{verbatim}
