...
\item Limited support for joins (only joins on row number).
...
equi-join tables on row number. One can join
tables of equal size by giving them all in the table list. One can
also join , for example, the main table of a
...
\begin{verbatim}
  SELECT FROM mytable,othertable
    WHERE not all(near(mytable.data, othertable.data))
\end{verbatim}
...
\paragraph*{}
A few basic functions and some specialized functions are available.
...
\item 'direction' gives one or more directions to convert. They can be
  ...
  \begin{itemize}
    \item As a constant scalar or array of strings giving one or more planetary objects
      like MOON or VENUS. In the future support for comets might be
      added.
...
\\Epochs can be given in two ways:
  \begin{itemize}
...
\begin{verbatim}
  ...
  date('9Oct2011/12:00:00'), 'UTC'   # given UTC time
\end{verbatim}
